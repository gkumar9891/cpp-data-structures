...
void sort2DArrayonPriority(vector<vector<int>> &arr) {
...
sort2DArrayonPriority(contests);
...
int i = 0;
while(totalImpLose != k) {
    if(totalImpLose < k || totalImpLose == 0) {
        break;
    }
    totalLuck = totalLuck - (2*contests[i][0]);
    i++;
    totalImpLose--;
}
